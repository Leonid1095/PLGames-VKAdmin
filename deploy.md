> ⚠️ **Устарело.** Гайд описывает старый однопользовательский режим (`VK_TOKEN`, Docker).
> Актуальный деплой — systemd-служба `vkadmin.service` (`uvicorn web.app:app`), подключение
> групп через `/dashboard` — см. README.
