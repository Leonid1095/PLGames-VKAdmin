...
async def write_patch_notes(
    ...
    system_prompt = (
        ...
        "Напиши красивый, структурированный патч-нот (список изменений) для поста на стене ВК. "
        "Группируй изменения по категориям (Новые функции, Исправления, Улучшения и т.д.). "
        "Используй эмодзи для категорий. Пиши понятно для обычных пользователей, "
        "не для программистов. Если коммит технический (рефакторинг, CI) — пропусти его. "
        "Никогда не добавляй хэштеги."
    )
    ...
    return await generate_response(
        prompt=(
            ...
            f"{commits_data}\n\n"
            "На основе этих коммитов напиши красивый патч-нот для поста в группе ВКонтакте."
        ),
        system_prompt=system_prompt,
        group_id=group_id,
    )
